...
>
  ...
  `,e([me({type:Boolean})],Ct.prototype,"open",void 0),Ct=e([pe("chorus-help")],Ct);const At={CC:"Soundbar",LF:"Front L",RF:"Front R",LR:"Rear L",RR:"Rear R",SW:"Sub"},Mt={CC:"var(--chorus-cc)",LF:"var(--chorus-front)",RF:"var(--chorus-front)",LR:"var(--chorus-rear)",RR:"var(--chorus-rear)",SW:"var(--chorus-sub)"};let Rt=class extends de{constructor(){super(...arguments),this.narrow=!1,this._view="editor",this._loading=!0,this._help=!1,this._polling=!1,this._onPop=()=>this._applyPath(this._currentPath())}firstUpdated(){this._load()}connectedCallback(){super.connectedCallback(),this._pollTimer=window.setInterval(()=>{this._poll()},12e3),window.addEventListener("popstate",this._onPop),this._applyPath(this._currentPath())}disconnectedCallback(){super.disconnectedCallback(),this._pollTimer&&(window.clearInterval(this._pollTimer),this._pollTimer=void 0),window.removeEventListener("popstate",this._onPop)}willUpdate(e){e.has("route")&&this._applyPath(this._currentPath())}_prefix(){return this.route?.prefix||"/chorus"}_currentPath(){if(null!=this.route?.path)return this.route.path;const e=window.location.pathname,t=this._prefix();return e.startsWith(t)?e.slice(t.length):""}_applyPath(e){const{view:t,room:r}=function(e){const t=(e||"").split("/").filter(Boolean);return{view:"overview"===t[0]?"overview":"editor",room:"editor"===t[0]&&t[1]?decodeURIComponent(t[1]):void 0}}(e);this._view=t,this._editRoom=r}_go(e,t){const r=this._prefix()+function(e,t){let r=`/${e}`;return"editor"===e&&t&&(r+=`/${encodeURIComponent(t)}`),r}(e,t);window.location.pathname!==r&&(history.pushState(null,"",r),this.dispatchEvent(new CustomEvent("location-changed",{bubbles:!0,composed:!0}))),this._view=e,this._editRoom=t}async _poll(){if(!this._polling&&this.hass){this._polling=!0;try{this._graph=await this.hass.connection.sendMessagePromise({type:"chorus/refresh"})}catch{}finally{this._polling=!1}}}async _load(e=!1){this._loading=!0,this._error=void 0;try{this._graph=await this.hass.connection.sendMessagePromise({type:e?"chorus/refresh":"chorus/bond_graph"})}catch(e){this._error=e?.message||e?.code||"unknown error"}finally{this._loading=!1}}render(){return I`
    ...
    ></chorus-editor>`}_overview(){if(this._loading&&!this._graph)return I`<div class="msg">Reading your speakers…</div>`;if(this._error)return I`<div class="msg err">Couldn't load the speaker graph: ${this._error}</div>`;const e=Pe(this._graph),t=e.filter(e=>e.key!==Ee),r=e.find(e=>e.key===Ee)?.tray??[];if(!t.length&&!r.length)return I`<div class="msg">No Sonos speakers discovered yet.</div>`;const o=[...t].sort((e,t)=>this._roomRank(e)-this._roomRank(t)||e.name.localeCompare(t.name,void 0,{numeric:!0}));return I`
      <div class="grid">
        ${o.map(e=>this._roomCard(e))}
        ${r.length?this._freeSubsCard(r):q}
      </div>
      ${this._supportNote()}
    `}_freeSubsCard(e){const t={label:1===e.length?"Unbonded sub":"Unbonded subs",entries:e.map(e=>({ch:"SW",name:this._spName(e),model:e.model}))};return I`
      <div class="card">
        <div class="rhead">
          ...
          <div class="rmeta">
            ...
            <span class="rsum"
              >${1===e.length?"1 sub":`${e.length} subs`} · not bonded to a set</span
            >
          </div>
        </div>
        <div class="groups">${this._groupBox(t)}</div>
      </div>
