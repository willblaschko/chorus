...
def _register_services(hass: HomeAssistant, coordinator: ChorusCoordinator) -> None:
    ...
    async def run(fn, *args):
        try:
            ...
            if err.code:
                # Surface the device's own errorDescription when present — the code alone
                # ("1034") is opaque; the description says what the speaker actually rejected.
                detail = f": {err.description}" if err.description else ""
                raise HomeAssistantError(f"Sonos rejected it (code {err.code}{detail}).") from err
